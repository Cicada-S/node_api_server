...
1. 在 `router/article.js` 模块中， 添加 `获取文章列表数据` 的路由：
...
2. 在 `router_handler/article.js` 模块中，定义并向外共享 `获取文章列表数据` 的路由处理函数：
...
1. 定义查询文章列表数据的 SQL 语句

```js
// 定义查询 文章是否被删除的 sql 语句
const sql = 'select * from ev_articles where is_delete = 0'
```

2. 执行 SQL 语句 将查询到的数据返回给客户端
...
```js
// 删除文章的路由
router.get('/deleteicle/:id', articleHandler.deleteIcleById)
```
...
```js
// 导入删除文章的验证规则对象
const { deleteIcleSchema } = require('../schema/article')
...
2. 调用 `db.query()` 执行删除文章数据的 SQL 语句：
...
```js
// 获取文章详情的路由
router.get('/articleinfo/:id', articleHandler.getArticleById)
```

2. 在 `/router_handler/article.js` 模块中，定义并向外共享 `根据 Id 获取文章数据` 的路由处理函数：
...
```js
// 验证规则对象 - 获取文章详情（因为和删除文章一样只需要验证id，就使用了同一个验证对象）
exports.articleIdSchema = {
  params: {
    id: cate_id
  }
...
```js
// 导入根据 Id 获取文章的验证规则对象
const { articleIdSchema } = require('../schema/article')
...
1. 在 `/router/article.js` 模块中，添加 `更新文章信息` 的路由：

```js
// 更新文章信息的路由
router.get('/updateicle/:id', articleHandler.updateIcleById)
```

2. 在 `/router_handler/article.js` 模块中，定义并向外共享 `更新文章信息` 的路由处理函数：
...
exports.updateArticleSchema = {
  body: {
    id: cate_id, // 文章id 但是使用的是cate_id的验证规则
    cate_id,
    title,  
    content,
    state
...
```js
// 导入更新文章信息的验证规则对象
const { updateArticleSchema } = require('../schema/article')
